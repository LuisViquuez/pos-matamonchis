...
async function setupDatabase() {
  try {
    ...
    // Ejecutar el SQL completo usando template literal
    ...
    const promotionCount = await sql`SELECT COUNT(*) as count FROM promotions`;

    console.log(`   - Usuarios: ${userCount[0].count}`);
    console.log(`   - Productos: ${productCount[0].count}`);
    console.log(`   - Clientes: ${customerCount[0].count}`);
    console.log(`   - Promociones: ${promotionCount[0].count}`);
